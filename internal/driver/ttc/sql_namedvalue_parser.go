	...
	for i := 0; i < n; i++ {
		...
		if ch == ':' {
			...
			if start >= n {
				...
				return nil, common.NewOracleError(oracleErrors.StatementParsingDanglingColon, nil)
			}
			...
			if _, found := occ[name]; !found {
				if len(uniqueNames) > maxBindIndex {
					return nil, common.NewOracleError(oracleErrors.StatementParsingInvalidArgCount, nil, len(uniqueNames)+1, maxBindIndex+1)
				}
				uniqueNames[name] = uint16(len(uniqueNames))
			}
			if len(binds) > maxBindIndex+1 {
				return nil, common.NewOracleError(oracleErrors.StatementParsingPlaceholdersArgsMismatch, nil, len(binds), maxBindIndex+1)
			}
...
func extractInputBindValues(bindDetails *bindDetails, in []driver.NamedValue) ([]driver.Value, error) {
	...
	placeByKey := func(key string, src driver.NamedValue) error {
		...
		if !ok {
			...
			return common.NewOracleError(oracleErrors.StatementParsingNameNotFound, nil, src.Name)
		}
		for _, idx := range indices {
			if filled[idx] {
				...
				return common.NewOracleError(oracleErrors.StatementParsingDuplicateArg, nil, bindDetails.bindNames[idx], int(idx)+1)
			}
	...
	for _, nv := range in {
		...
		if nv.Name != "" {
			...
			if nv.Ordinal <= 0 || nv.Ordinal > numberOfBinds {
				...
				return nil, common.NewOracleError(oracleErrors.StatementParsingInvalidOrdinal, nil, nv.Ordinal, numberOfBinds)
	...
	for _, nv := range in {
		...
		if nv.Name != "" {
			...
			if !ok {
				...
				return nil, common.NewOracleError(oracleErrors.StatementParsingNameNotFound, nil, nv.Name)
			}
			if filled[index] {
				common.Odl.Error("extractInputBindValuesForPlSql: duplicate value for placeholder", "placeholder", nv.Name, "position", index+1)
				return nil, common.NewOracleError(oracleErrors.StatementParsingDuplicateArg, nil, nv.Name, int(index)+1)
			}
			...
			if filled[index] {
				...
				return nil, common.NewOracleError(oracleErrors.StatementParsingDuplicateArg, nil, bindDetails.bindNames[index], nv.Ordinal)
			}
